...
std::string DeduplicationInfo::debug() const
{
    ...
    return fmt::format(
        ...
        size_t(this), getRows(), original_block.rows(), fmt::join(parts, ","), getCount(), fmt::join(token_strs, ","), visited_views.size(), fmt::join(visited_views, ","), last_partition_choice);
}
